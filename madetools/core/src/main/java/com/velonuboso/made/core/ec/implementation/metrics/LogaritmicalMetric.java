 ...
 * @author Ruben
 */
public class LogaritmicalMetric implements IFitnessMetric {
    ...
    public TrialInformation getTrialInformation(ArrayList<WorldDeductions> deductionsForAllTrials) {
        ...
        for (WorldDeductions deductionsForTrial : deductionsForAllTrials){
            float sumatory = 0; 
            for (Trope trope: deductionsForTrial.keySet()){
                int numberOfDeductionsForTrope = deductionsForTrial.get(trope).length;
                float logaritmicValue = (float) Math.log10(numberOfDeductionsForTrope+1.0);
                sumatory += logaritmicValue;
            }
            fitnessPerTrial.add(sumatory);
        }
